...
| Method | Endpoint | توضیح | Auth |
|--------|----------|-------|------|
| `POST` | `/` | ارسال گزارش یا بازخورد | ✅ |

**نکات مهم**:
- کاربران می‌توانند گزارش عمومی (بازخورد)، گزارش کاربر یا گزارش کارت ارسال کنند
- فیلد `body` الزامی است (حداقل 10 کاراکتر)
- فیلدهای `reported_id` و `card_id` اختیاری هستند
- گزارش‌ها در پنل ادمین قابل مشاهده و بررسی هستند
...
| `POST` | `/backups/upload` | آپلود فایل بکاپ (حداکثر 500MB) | ✅ Admin |
| `POST` | `/backups/{filename}/restore` | بازگردانی دیتابیس از بکاپ | ✅ Admin |
...
- آپلود فایل بکاپ (فرمت‌های `.sql.gz`, `.gz`, `.sql`)
- بازگردانی دیتابیس از بکاپ با ایجاد بکاپ امنیتی خودکار قبل از restore
- حداکثر حجم فایل آپلود: 500 مگابایت
- بکاپ‌گیری اتوماتیک روزانه

**ادمین پیش‌فرض (Recovery)**:
- در صورت نبودن هیچ ادمینی، سیستم یک ادمین پیش‌فرض ایجاد می‌کند
- ایمیل و پسورد از متغیرهای محیطی `DEFAULT_ADMIN_EMAIL` و `DEFAULT_ADMIN_PASSWORD` خوانده می‌شود
- این ویژگی برای recovery از crash یا از دست رفتن دیتا مفید است

**هشدارهای Startup**:
- سیستم در هر startup سلامت دیتابیس را بررسی می‌کند
- در صورت شناسایی مشکل (crash، دیتای از دست رفته)، ایمیل به ادمین‌ها ارسال می‌شود
